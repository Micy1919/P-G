...
storage_all_file_path = 'storage_all_' + nation + '_' + ntype + '.tsv'
storage_all_file_path_tmp = 'storage_all_' + nation + '_' + ntype + '_tmp.tsv'

ban_urls = ['www.ligaolahraga.com/jadwal-bola']

twitter_crawlers = ['twitter_influencers_crawler']

logging.basicConfig(filename='logger_%s_%s.log'%(nation, ntype), level=logging.INFO)


class NewsStreamingConsumer:
    def __init__(self, nation):
        group_id = ('news_streaming_clustering_prime_%s_%s_%s' % (nation, lan, ntype))
        config_sever_key = ('%s_new' % kafka_mapnation)
...
if __name__ == '__main__':
    top_domains = set()
    with open('top_domains_{}_{}.tsv'.format(nation, lan)) as lines:
        for line in lines:
            domain = line.strip()
