...
class PreProcess(object):
    def __init__(self, punctuation_file, stopwords_file, special_terms=["_line_"]):
        ...
        self.stopwords = set([line.strip().decode('utf-8') for line in open(stopwords_file, "r")])
        self.special_terms = special_terms
        self.__tokens = None
        self.__content = []
        self.grammars = ["NP: {<S.*>}", "NP: {<A|S.*><S.*>}", "NP: {<S.*><S.*|N.*>}","NP: {<S.*><IN|S.*><S.*>}"]
    ...
    def __get_noun_chunks(self):
        ...
        :return:
        """
        tagged_words = [(w[2],w[1]) for w in self.__content]
        noun_chunks = []
