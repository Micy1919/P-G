...
class PreProcess(object):
    ...
    def __key_pre(self, word):
        ...
        """
        if len(word) == 1:
            if self.__num_ratio(word[0], 0.6):
                return []
            return word
        elif len(word) == 0:
            return []
            ...
            for i in range(len(word)-1):
                if i == 0 and self.__num_ratio(word[i],0.6):
                    return self.__key_pre(word[1:])
                token.append(word[i])
            token.append(word[-1])
            return token

    def __key_splitsentence(self, content):
        """
        ...
        sentences = self.tokenizer.tokenize(content.decode('utf-8'))
        return sentences
    ...
    def text_rank_process(self, text):
        """
        生成text_rank需要的文本
        :param text:
        :return:
        """
        terms = self.__key_splitsentence(text.lower())
        terms = [self.text_process(w.encode('utf-8')) for w in terms]
        return terms

if __name__ == "__main__":
    text = "India vs Australia: Ravichandran Ashwin breaks Dale Steyn\u2019s record of most wickets in a Test season_line_ddddd six 7s 777s"
